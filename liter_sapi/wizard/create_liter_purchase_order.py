...
class create_liter_purchase_order(models.TransientModel):
    _name = "create.liter.purchase.order"

    def create_purchase_order(self):
        active_ids = self._context.get('active_ids')
        list_of_ids  = []
        lab_req_obj = self.env['liter.sapi']
        purchase_order_obj  = self.env['purchase.order']
        purchase_order_line_obj = self.env['order.line']
        ir_property_obj = self.env['ir.property']
        for active_id in active_ids:
            lab_req = lab_req_obj.browse(active_id)
            lab_req.validity_status = 'purchase'
            if lab_req.no_purchase == False:
                invoice_vals = {
