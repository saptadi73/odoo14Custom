...
class AccountMoveLine(models.Model):
    ...
    multi_payment_id = fields.Many2one('multi.payment.line', string='Multi Payment Id')
...
class Accountpayment(models.Model):
    ...
    payment_line_ids = fields.One2many('multi.payment.line', 'account_payment_id')


    def action_post(self):
        if self.payment_for :
            if self.payment_line_ids:
                if self.amount != sum(self.payment_line_ids.mapped('allocation')):
    ...
    def dev_generate_moves(self):
        if self.payment_for :
            ...
            for line in self.payment_line_ids:
                ...
                move_line = self.env['account.move.line'].search([('move_id','=',self.move_id.id),('multi_payment_id','=',line.id)])
    ...
    def load_payment_lines(self):
        if self.payment_for :
            ...
            for vals in invoice_ids:
                ...
                query = """ INSERT INTO multi_payment_line (invoice_id, account_id, date, due_date, original_amount, balance_amount, currency_id, account_payment_id,tipe_invoice) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
...
class AdvanceeMultiPayment(models.Model):
