...
class create_appointments_bill_wizard(models.TransientModel):
    _name = "medical.appointments.bill.wizard"
    ...
    def create_bill(self):
        ...
        for active_id in active_ids:
            ...
            lab_req.validity_status_bill = 'bill'
            lab_req.state = 'done'
            if lab_req.is_invoiced  == True:
                raise Warning('All ready Billed.')
            if lab_req.no_invoice == False:
                ...
                'partner_shipping_id':lab_req.doctor_id.partner_id.id,
                'currency_id':lab_req.doctor_id.partner_id.currency_id.id ,
                'invoice_payment_term_id': False,
                'fiscal_position_id': lab_req.doctor_id.partner_id.property_account_position_id.id,
                'team_id': False,
                ...
                if lab_req.consultations_id.id:
                    invoice_line_account_id = lab_req.consultations_id.property_account_expense_id.id or lab_req.consultations_id.categ_id.property_account_expense_categ_id.id or False
                ...
                tax_ids = taxes.ids
                invoice_line_vals = {
                    'name': lab_req.name or '',
                    'account_id': invoice_line_account_id,
                    'price_unit':lab_req.consultations_id.standard_price,
                ...
                if list_of_ids:
                        ...
                        action = imd.sudo().xmlid_to_object('account.action_move_in_invoice_type')
                ...
                raise UserError(_(' The Appointment is bill exempt   '))
            return result
