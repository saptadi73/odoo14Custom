    ...
    def create_invoice(self):
        ...
        for active_id in active_ids:
            ...
            if lab_req.no_invoice == False:
                ...
                invoice_vals = {
                'name': self.env['ir.sequence'].next_by_code('medical_inp_inv_seq'),
                ...
                if lab_req.hospitalized_id.id:
                    invoice_line_account_id = lab_req.hospitalized_id.property_account_income_id.id or lab_req.hospitalized_id.categ_id.property_account_income_categ_id.id or False
                if not invoice_line_account_id:
                    inc_acc = ir_property_obj.get('property_account_income_categ_id', 'product.category')
                if not invoice_line_account_id:
                    raise UserError(
                        _('There is no income account defined for this product: "%s". You may have to install a chart of account from Accounting app, settings menus.') %
                        (lab_req.hospitalized_id.name,))

                tax_ids = []
                taxes = lab_req.hospitalized_id.taxes_id.filtered(lambda r: not lab_req.hospitalized_id.company_id or r.company_id == lab_req.hospitalized_id.company_id)
                tax_ids = taxes.ids
                invoice_line_vals = {
                    'name': lab_req.hospitalized_id.name or '',
                    'account_id': invoice_line_account_id,
                    'price_unit':lab_req.hospitalized_id.lst_price,
                    'product_uom_id':lab_req.hospitalized_id.uom_id.id,
                    'quantity': 1,
                    'product_id':lab_req.hospitalized_id.id,
                }
                ...
                raise UserError(_(' The Hospitalized is invoice exempt   '))
            return result
