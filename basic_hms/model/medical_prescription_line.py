...
class medical_prescription_line(models.Model):
    ...
    metode_pengobatan = fields.Selection([
        ('im', 'Intra Mosculer (Suntik dibagian daging)'),
        ('iv', 'Intrea Vena ( Suntik di pembuluh darah )'),
        ('sc', 'Suntik dibawah kulit'),
        ('iu', 'Intra Uteri ( Melalui kandungan )'),
        ('intra_mame', 'Intra Mammae ( Melalui Putting Susu )'),
        ('local', 'Local ( Dioles ke kulit )'),
    ], 'Metode Pengobatan')
    medical_patient_prescription_id = fields.Many2one('medical.patient',string='Medication')
    medical_patient = fields.Many2one('medical.patient', string='Patient')
