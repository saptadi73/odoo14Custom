...
class HrPayslip(models.Model):
    ...
    def get_worked_day_lines(self, contracts, date_from, date_to):
        ...
        for contract in contracts.filtered(lambda contract: contract.resource_calendar_id):
            ...
            contract_start = contract_end = False
            cont_start = datetime.combine(fields.Date.from_string(contract.date_start), time.min)
            if cont_start>day_from:
                contract_start = cont_start
                day_from = contract_start

            if contract.date_end:
                contract_end = datetime.combine(fields.Date.from_string(contract.date_end), time.min)
                day_to = contract_end
            
            work_data2 = contract.employee_id.with_context(no_tz_convert=True).get_work_days_data(day_from, day_to, calendar=contract.resource_calendar_id)
            attend = {
                    'name': _("Attended Working Days paid at Prorate"),
                    'sequence': 1,
                    'code': 'ATTEND100',
                    'number_of_days': work_data2['days'],
                    'number_of_hours': work_data2['hours'],
                    'contract_id': contract.id,
                    }
            res.append(attend)
#                raise UserError(_('date start %s == %s')%(contract_start,day_from,))	
