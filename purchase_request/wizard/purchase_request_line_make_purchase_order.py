...
#                raise UserError(_('item %s')%(self._prepare_item(line)))
    ...
    def default_get(self, fields):
        ...
        if len(res["item_ids"])==0:
            raise UserError(_('All items already Requested For Quotations'))
...
#                all_qty = min(po_line_product_uom_qty, wizard_product_uom_qty)
                all_qty = po_line_product_uom_qty + wizard_product_uom_qty
...
#            raise UserError(_('new qty %s \n po qty %s \n wiz qty %s')%(new_qty,po_line_product_uom_qty,wizard_product_uom_qty))
