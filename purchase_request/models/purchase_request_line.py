...
class PurchaseRequestLine(models.Model):
    ...
    @api.onchange("request_id.date_start")
    def _onchange_date_start(self):
        if self.request_id.date_start:
            self.date_required = self.request_id.date_start
...
#        compute="_compute_estimate_cost",
    ...
    laktasi_id = fields.Many2one('master.status.laktasi', 'Status Laktasi')
    sapi_ids = fields.Many2many('sapi', string='Sapi')

    @api.onchange('laktasi_id')
    def onchange_laktasi_id(self):
        if self.laktasi_id:
            # Ambil stts_laktasi_id dari laktasi_id yang dipilih
            laktasi_id = self.laktasi_id.id

            # Filter sapi yang memiliki stts_laktasi_id yang sama
            sapi_records = self.env['sapi'].search([('stts_laktasi_id', '=', laktasi_id)])

            # Set nilai pada sapi_ids dengan record sapi yang sesuai
