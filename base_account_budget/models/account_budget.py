...
class BudgetLines(models.Model):
    ...
    planned_amount = fields.Float('Planned Amount', required=True, digits=0, compute='_compute_total_planned')
    ...
    juni = fields.Float()
    juli = fields.Float()
    agustus = fields.Float()
    sep = fields.Float()
    nov = fields.Float()
    okt = fields.Float()
    des = fields.Float()
    # Encumbren
    januari_en = fields.Float()
    februari_en = fields.Float()
    maret_en = fields.Float()
    april_en = fields.Float()
    mei_en = fields.Float()
    juni_en = fields.Float()
    juli_en = fields.Float()
    agustus_en = fields.Float()
    sep_en = fields.Float()
    okt_en = fields.Float()
    nov_en = fields.Float()
    des_en = fields.Float()
    # Actual
    januari_ac = fields.Integer()
    februari_ac = fields.Integer()
    maret_ac = fields.Integer()
    april_ac = fields.Integer()
    mei_ac = fields.Integer()
    juni_ac = fields.Integer()
    juli_ac = fields.Integer()
    agustus_ac = fields.Integer()
    sep_ac = fields.Integer()
    okt_ac = fields.Integer()
    nov_ac = fields.Integer()
    des_ac = fields.Integer()


    @api.depends('jan','feb','mar','april', 'mei','juni','juli','agustus','sep','nov','des')  # jalankan method ini, jika nilai jumlah dan harga berubah
    def _compute_total_planned(self):
        for record in self:
            record.planned_amount = record.jan+record.feb+record.mar+record.april+record.mei+record.juni+record.juli+record.agustus+record.sep+record.okt+record.nov+record.des

    # @api.depends('januari_en', 'februari_en', 'maret_en', 'april_en', 'mei_en', 'juni_en', 'juli_en', 'agustus_en', 'sep_en', 'nov_en',
    #              'des_en')  # jalankan method ini, jika nilai jumlah dan harga berubah
    # def _compute_total_planned(self):
    #     for record in self:
    #         record.planned_amount = record.januari_en + record.februari_en + record.maret_en + record.april_en + record.mei_en + record.juni_en + record.juli_en + record.agustus_en + record.sep_en + record.okt_en + record.nov_en + record.des_en
