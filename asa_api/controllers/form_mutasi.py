...
class SapiApi(http.Controller):
	...
	@http.route('/create_form_mutasi', auth="none", type='json',cors='*')
	def create_form_mutasi(self, **rec):
		try:
			...
			if uid:
				if request.jsonrequest :
					...
					if rec.get('tgl_layanan') :
						...
						tgl_layanan = False
					if rec.get('id_jenis_mutasi') :
						id_jenis_mutasi = rec['id_jenis_mutasi']
					else :
						id_jenis_mutasi = False
					if rec.get('mati') :
						mati = rec['mati']
					else :
						mati = False
					...
					if rec.get('alamat2') :
						...
						alamat2 = False
					if rec.get('penerimaan') :
						penerimaan = rec['penerimaan']
					else :
						penerimaan = False
					if rec.get('bertambah') :
						bertambah = rec['bertambah']
					else :
						bertambah = False
					...
					new_record = request.env['form.mutasi'].sudo().create(value_data)
					new_record._onchange_peternak_id()
					new_record._onchange_petugas_id()
					new_record._onchange_tgl_layanan()

					if new_record :
						return ({'result':'Create Form Mutasi Success...!!', "ID Layanan": new_record.id})
					else :
						return ({'result':'Create Form Mutasi Gagal...!!'})
	...
	@http.route('/get_master_jenis_mutasi', auth="none", type='json',cors='*')
	def get_master_jenis_mutasi(self, **rec):
		try:
			if request.jsonrequest :
				master = request.env['master.jenis.mutasi'].sudo().search([])
				data_master = []
				for m in master:
					value = { 	'id'        			: m.id,
								'kode'					: m.kode or None,
								'jenis_mutasi' 			: m.jenis_mutasi or None,
									   
									  }
		
					data_master.append(value)
				return ({"message": "List Master Jenis Mutasi", "Data": data_master})
			else :
				return ({'result':'Failed Check Your Parameter'})
		except Exception as e:
			return {'status': False, 'error': str(e)}

	@http.route('/select_data_mutasi', auth="none", type='json',cors='*')
	def select_data_mutasi(self, **rec):
		try:
			header = request.httprequest.headers
			db= header.get('db')
			login = header.get('login')
			...
			if uid:
				if request.jsonrequest :
					if rec.get('id_layanan'):
						layanan = request.env['form.mutasi'].search([('id','=',rec.get('id_layanan'))])
						data_layanan = []
						if layanan :
