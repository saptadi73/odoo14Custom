...
class SapiApi(http.Controller):
	...
	@http.route('/create_form_abortus', auth="none", type='json',cors='*')
			...
			if uid:
				if request.jsonrequest :
					...
					if rec.get('tgl_layanan') :
						...
						tgl_layanan = False
					if rec.get('penyebab_abortus_id') :
						penyebab_abortus_id = rec['penyebab_abortus_id']
					else :
						penyebab_abortus_id = False
					if rec.get('masa_bunting') :
						masa_bunting = rec['masa_bunting']
					else :
						masa_bunting = False
					if rec.get('komentar') :
					...
					# sapi = request.env['sapi'].sudo().search([('id','=',id_sapi)],limit=1)
					value_data = {  'peternak_id': peternak_id,
									...
									'id_sapi': id_sapi,
									'eartag_id': eartag,
									'penyebab_abortus_id': penyebab_abortus_id,
									'masa_kebun': masa_bunting,
									'komentar': komentar,
									'bcs': bcs
									  }

					new_record = request.env['form.abortus'].sudo().create(value_data)
					new_record._onchange_peternak_id()
					new_record._onchange_petugas_id()
					new_record._onchange_tgl_layanan()
					
					if new_record :
						return ({'result':'Create Form Abortus Success...!!', "ID Layanan": new_record.id})
					else :
						return ({'result':'Create Form Abortus Gagal...!!'})
	...
	@http.route('/get_master_abortus', auth="none", type='json',cors='*')
	def get_master_abortus(self, **rec):
		try:
			if request.jsonrequest :
				master = request.env['master.abortus'].sudo().search([])
				data_master = []
				for m in master:
					value = { 	'id'        				: m.id,
								'kode'						: m.kode or None,
								'nama_abortus' 				: m.nama_abortus or None,
									   
									  }
		
					data_master.append(value)
				return ({"message": "List Master Abortus", "Data": data_master})
			else :
				return ({'result':'Failed Check Your Parameter'})
		except Exception as e:
			return {'status': False, 'error': str(e)}
	...
	@http.route('/select_abortus', auth="none", type='json',cors='*')
	def select_data_abortus(self, **rec):
		try:
			header = request.httprequest.headers
			db= header.get('db')
			login = header.get('login')
			...
			if uid:
				if request.jsonrequest :
					if rec.get('id_layanan'):
						layanan = request.env['form.abortus'].search([('id','=',rec.get('id_layanan'))])
						data_layanan = []
						if layanan :
							value = {   'Anggota/Peternak'    : layanan.peternak_id.peternak_name or None,
										...
										'catatan_petugas'     : layanan.komentar or None
