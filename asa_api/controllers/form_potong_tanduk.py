...
class SapiApi(http.Controller):
	...
	@http.route('/create_form_potong_tanduk', auth="none", type='json',cors='*')
	def create_form_potong_tanduk(self, **rec):
		try:
			...
			if uid:
				if request.jsonrequest :
					...
					new_record = request.env['form.pt'].sudo().create(value_data)
					new_record._onchange_peternak_id()
					new_record._onchange_petugas_id()
					new_record._onchange_tgl_layanan()
					if new_record :
						return ({'result':'Create Form Potong Tanduk Success...!!', "ID Layanan": new_record.id})
					else :
						return ({'result':'Create Form Potong Tanduk Gagal...!!'})
	...
	@http.route('/select_data_pot_tanduk', auth="none", type='json',cors='*')
	def select_data_pot_tanduk(self, **rec):
		try:
			header = request.httprequest.headers
			db= header.get('db')
			login = header.get('login')
			...
			if uid:
				if request.jsonrequest :
					if rec.get('id_layanan'):
						layanan = request.env['form.pt'].search([('id','=',rec.get('id_layanan'))])
						data_layanan = []
						if layanan :
